...
* ```filter_atTP_biggestN``` selects for a number of most abundant IS at a specified measurement.
* ```filter_atTP_min``` selects for IS that have at least a given number of
...
* ```filter_nrTP_min``` selects for IS that show in at least a given number of
  measurements.
...
similarity based on $R^2$ illustrated by two integration sites (19, 20)
originating from the same clone. They present a high $R^2$ similarity score.
Two integration sites originating from different clones (20, 21) do not show
this characteristic correlation:
...
To represet the clusterings produced by the k-Medoids clustering algortihm, we
...
* ```cloneCounts``` is the raw result from the clonal simulation (step 1),
* ```cloneReadouts``` is the result after applying noise to the clonal
  simulation (step 2),
* ```barcodeCount``` gives the raw IS readouts, i.e. after mapping ISs
  to the clones but before applying measurement noise (step 3),
* ```barcodeReadouts``` is the final result of our simulation routine
  (step 4). This adds measurement noise to the ```barcodeCounts``` and would
  correspond to the data that is available in a real-world experiment.

NB: here, the term "barcode" refers to integrations at individual and distinct
integration sites.
...
For the data from our simulation, the true association between IS annd clones is known:
